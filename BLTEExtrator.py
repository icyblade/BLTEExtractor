...
def extract(input_path, hash_in_filename=None):
    ...
    f.seek(0)
    if header_size > 0:
        blte_hash = md5(f.read(header_size))
    else:
        blte_hash = md5(f.read())
    ...
    for i in xrange(chunk_count):
        ...
        if header_size != 0:
            ...
            chunk_info_entry['checksum'] = string_to_hex(f.read(16))
        else:
    ...
    for chunk in chunks:
        ...
        if chunk['checksum'] and md5(chunk['data']) !=  chunk['checksum']:
            raise Exception('Chunk hash checksum mismatch')
        yield decode_chunk(chunk['data'])
    
def decode_chunk(data):
    flag = string_to_hex(data[0])
...
wow/data/2e/d8/2ed87af5044ed167e1fbc9d888ad3fdb').read())
    for decoded_chunk in extract('tmp', '2ed87af5044ed167e1fbc9d888ad3fdb'):
        print('%s %s' % (
            len(decoded_chunk),
            string_to_hex(decoded_chunk[:30])
        ))
